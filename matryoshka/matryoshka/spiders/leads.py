...
class LeadsSpider(scrapy.Spider):
    ...
    def load_main_page(self, response):
        ...
        yield scrapy.Request(url, self.load_pagination)

    def load_pagination(self, response):
        x_path = '//ul[@class="page"]/li/a[not(contains(@data-limit, "500"))]/@href'
        for url in response.xpath(x_path).getall():
            yield scrapy.Request(response.urljoin(url), self.scrape_page)
